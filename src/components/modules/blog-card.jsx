	...
	return (
		<HoverScaleAnimation>
			<Link href={`/blog/${slug}`} passHref>
				...
				>
					<div
						className={`relative h-35 w-full shrink-0 overflow-hidden rounded-md ${isFeatured === true ? 'h-65' : 'lg:h-40 lg:w-50'}`}
					>
						{imgSrc ? (
							<Image
								loader={imageLoader}
					...
					<div className=' flex flex-col justify-around gap-2'>
						<Card.Header className='flex flex-col gap-0.75'>
							...
							<Card.Title className='text-xl md:text-2xl font-jetbrains text-foreground/85 text-balance'>
								{title}
							</Card.Title>
						</Card.Header>
